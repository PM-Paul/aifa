...
The portfolio currently includes the MVP prototype, a Product Vision & Strategy, a Product Brief, and a Problem Discovery & Validation Plan. Together these artifacts document both the product as it exists today and the approach that will guide future product decisions through customer discovery.
...
The build was a deliberate first use of AI-assisted prototyping, chosen in a domain where the author already holds expertise so that the learning would produce a working, instrumented artifact rather than a toy. The process was fastest where the problem was already specified precisely, and demanded the most judgment where it was not: deciding what the product should refuse to answer, and where a deterministic calculation had to replace a plausible-sounding LLM-generated one.
