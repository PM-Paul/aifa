...
A primary objective of Stage 1 user interviews is to gather the operational data needed to refine or split these initial personas before advancing. The feedback loop will be monitored for clear split triggers, such as whether an architect’s workflow is driven by risk standardization (common in large Systems Integrators) or vendor commission margins (common in Value-Added Resellers). These insights will be used to map the exact internal procurement chains required for Stage 2\. 

| Segment | Persona | Why they have the problem |
| :---- | :---- | :---- |
| User | Client-facing Infrastructure Architect | Design cloud architectures and recommend infrastructure to external customers. Pricing and performance specs for GPUs change constantly across cloud providers. Manual calculations are slow, repetitive, and error-prone. |
...
**Excluded from MVP discovery.** To preserve maximum development velocity and sales focus, internal enterprise solution architects are explicitly excluded from this plan. While they perform similar calculations, internal IT teams lack the revenue-driving pressures of billable utilization, proposal turnaround speed, and external client-facing polish that make AI consultancies the highest-intent market for the MVP.
